...

{
	...
	*/
	...
	if (m_hComm == INVALID_HANDLE_VALUE)
	{
		...
		{
			LOGE(("COM%d ERROR_FILE_NOT_FOUND,Error Code:%d"), portnr, GetLastError());
			break;
		}
		//串口拒绝访问
		case ERROR_ACCESS_DENIED:
		{
			LOGE(("COM%d ERROR_ACCESS_DENIED,Error Code:%d"), portnr, GetLastError());
			break;
		}
		default:
			break;
...
void CSerialPort::ProcessErrorMessage(TCHAR* ErrorText)
{
	...
	LOGE(("WARNING:  %s Failed with the following error: \n%s\nPort: %d\n"), ErrorText, (TCHAR*)lpMsgBuf, m_nPortNr);
		...
		if (bWrite)
		{
			...
			if (!bResult)
			{
				...
				case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
				{
					...
					LOGE(("COM%d ERROR_ACCESS_DENIED，WriteFile() Error Code:%d"), port->m_nPortNr, GetLastError());
					break;
				}
				...
				case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
				{
					CloseHandle(port->m_hComm);
					port->m_hComm = INVALID_HANDLE_VALUE;
					LOGE(("COM%d ERROR_BAD_COMMAND，WriteFile() Error Code:%d"), port->m_nPortNr, GetLastError());
					break;
				}
				default:
				{
...
void CSerialPort::ReceiveChar(CSerialPort* port)
{
	...
	for (;;)
	{
		...
		if (bRead)
		{
			...
			if (!bResult)
			{
				...
				case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
				{
					CloseHandle(port->m_hComm);
					port->m_hComm = INVALID_HANDLE_VALUE;
					LOGE(("COM%d ERROR_ACCESS_DENIED，ReadFile() Error Code:%d"), port->m_nPortNr, GetLastError());
					break;
				}
				case ERROR_INVALID_HANDLE:///打开串口失败 erroe code:6
				{
				...
				case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
				{
					CloseHandle(port->m_hComm);
					port->m_hComm = INVALID_HANDLE_VALUE;
					LOGE(("COM%d ERROR_BAD_COMMAND，ReadFile() Error Code:%d"), port->m_nPortNr, GetLastError());
					break;
				}
				default:
				{
		...
		if (port->sendMessage)port->sendMessage->onSerialRecv(&RXBuff, port->m_nPortNr,(LPARAM)port->m_nPortNr);
...
void CSerialPort::ReceiveStr(CSerialPort* port)
{
	...
	for (;;)
	{
		...
		port->sendMessage->onSerialRecv(RXBuff, port->m_nPortNr, BytesRead);
...
{
	std::string m_version = "CSerialPort 3.0.3.180621 MOD cjn";
	return m_version;
}

void CSerialPort::SerialThread()
{
	///TRUE表示线程正在运行
	m_protecter.lock();
	m_bThreadAlive = TRUE;
	...
	for (;;)
	{
		...
		*/
		...
		bResult = WaitCommEvent(m_hComm, &Event, &m_ov);///表示该函数是异步的
		...
		if (!bResult)
		{
			...
			case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
			{
				CloseHandle(m_hComm);
				m_hComm = INVALID_HANDLE_VALUE;
				LOGE(("COM%d ERROR_ACCESS_DENIED，WaitCommEvent() Error Code:%d"), m_nPortNr, GetLastError());
				break;
			}
			case ERROR_INVALID_HANDLE:///打开串口失败 erroe code:6
			{
			...
			case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
			{
				CloseHandle(m_hComm);
				m_hComm = INVALID_HANDLE_VALUE;
				LOGE(("COM%d ERROR_BAD_COMMAND，WaitCommEvent() Error Code:%d"), m_nPortNr, GetLastError());
				break;
			}
		...
		if (Event >= WAIT_OBJECT_0 && Event < WAIT_OBJECT_0 + 3)
		{
			...
			{
				if (WaitForSingleObject(m_hEventArray[i], 0) == WAIT_OBJECT_0)
				{
					...
					case WAIT_OBJECT_0 + 2:	// read event 将定义的各种消息发送出去
					{
						...
						if (CommEvent & EV_RXCHAR) //接收到字符，并置于输入缓冲区中
						{
							...
							{
								ReceiveStr(this);//多字符接收
							}
							else if (IsReceiveString == 0)
							{
								ReceiveChar(this);//单字符接收
							}
							else
							{
								//默认多字符接收
